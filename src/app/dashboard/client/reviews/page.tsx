...
export default async function ClientReviewsPage() {
  ...
  if (user?.email) {
    try {
      myReviews = (await getClientReviews(user.email)) || [];
    } catch (err) {
    ...
    <div className="space-y-6 w-full animate-pulse">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
        {[...Array(6)].map((_, index) => (
          <div
            key={index}
            className="card-surface p-5 rounded-2xl flex flex-col justify-between space-y-4 h-[190px]"
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="space-y-1.5 w-3/4">
                  <div className="h-3 bg-content2 w-1/3 rounded" />
                  <div className="h-4 bg-default-200 w-4/5 rounded-md" />
                </div>
                <div className="h-6 bg-default-200 rounded-lg w-10 shrink-0" />
              </div>

              <div className="bg-content2 dark:bg-content2/5 p-3 rounded-xl border border-border/50 space-y-2 h-[75px]">
                <div className="h-3 bg-default-200 w-full rounded" />
                <div className="h-3 bg-default-200 w-5/6 rounded" />
                <div className="h-3 bg-content2 w-1/2 rounded" />
              </div>
            </div>

            <div className="flex items-center justify-between pt-2 border-t border-border/60 w-full">
              <div className="h-3 bg-content2 w-1/4 rounded" />
